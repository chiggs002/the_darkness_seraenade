...
//
//   uses osc data to set out messages
//
//
//   THIS PATCH DUMMIES THE KINECT INPUT FOR TESTING THE COMPLETE INSTALLATION
//
//   SENDS ALL OUTPUTS TO MAXMSP AND PROCESSING ON PORT 6000 AND 6001
//
...
//
//
//   Sends out osc data on port 6000 AND 6001 on the local network
//   NetAddress("127.0.0.1",6000);
//
//
//
...
OscP5 oscP5;
OscP5 oscP6;
NetAddress dest_Proc,dest_Max1,dest_Max2,dest_Max3,dest_Max4,dest_Max5,dest_WekHelp,dest_Wek;
...
PImage  img;
float minThresh=200;
float maxThresh=890; //900-1000? when testing
...
void setup() {
  ...
  dest_WekHelp= new NetAddress("127.0.0.1",6448);
   dest_Wek= new NetAddress("127.0.0.1",6449);
 ...
 ////TRACK THE SWARM
  ...
  // v.applyBehaviors(vehicles,int(avgX),int(avgY));
   ...
   //v.applyBehaviors(vehicles,rhx,rhy);
    ...
    v.applyBehaviors(vehicles,mouseX,mouseY);
...
if ( (distToTop <200) && (distToBottom <200) && (distToRight <200) && (distToLeft <200) ) {
  oscCorners=0;
