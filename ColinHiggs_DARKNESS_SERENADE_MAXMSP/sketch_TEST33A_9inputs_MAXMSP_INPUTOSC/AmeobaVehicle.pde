...
//
//
//   CREATE SWARM OF AMEOBA PARTILCES
//
...
class AmeobaVehicle {
  ...
  color interA;
    ...
    AmeobaVehicle(float x, float y, int whatkind, float mmaxspeed, float mmaxforce) {
    moveTimer=0;
   timeToLerp=0;
  ...
  void applyBehaviors(ArrayList<AmeobaVehicle> vehicles,ArrayList<DarkVehicle> darkvehicles, int Mx,int My) {
...
PVector seekRandom(PVector target) {
  ...
  PVector separate (ArrayList<AmeobaVehicle> vehicles) {
    float desiredseparation = radius*2;
    PVector sum = new PVector();
    int count = 0;
    // For every boid in the system, check if it's too close
    for (AmeobaVehicle other : vehicles) {
  ...
  void display(int lerpperiod) {
   ...
   void changecol(ArrayList<AmeobaVehicle> vehicles,int mX,int mY ) {
