...
startAmeobaCounter=1000;
startAmeoba02Counter=950;
...
void draw() {
  ...
  if (oscMixGraphics == 1) {
    ...
    startAmeobaCounter=340;
    startAmeoba02Counter=650;
    
  } else if (oscMixGraphics == 2) {
     startEraticCounter=150;
    startAmeobaCounter=750;
    startAmeoba02Counter=650;
    
  } else if (oscMixGraphics == 3) {
      startEraticCounter=50;
    startAmeobaCounter=650;
    startAmeoba02Counter=350;
...
//ellipse(width/2.0,height/2.0,200,200);
  ...
  //println("signal value =====  ",oscFloatSignal);
   ...
   ellipse(width/2.0+30,height/2.0-10,250,250);
   fill(255,255,60);
   textAlign(CENTER);
   PFont font;
   font = createFont("DIN-Black.otf",32);
   textFont(font, 32);
   textSize(25);
   text("TO SWITCH ON ", (width/2.0)+35, (height/2.0)-29,0); 
   text("STAND HERE ", (width/2.0)+35, (height/2.0)+29,0);
   
  //Fill();
  
  println("FUCK WHER ARE U");
    //fill(0,250,0,255);//176 221 238
    //noFill();
//ellipse(width/2.0,height/2.0,332,332);
popStyle();
popMatrix();
...
void oscEvent(OscMessage theOscMessage) {
 ...
 if (theOscMessage.checkAddrPattern("/wek/outputs")==true) {
     if(theOscMessage.checkTypetag("fffffffffffffffff")) { // looking for 4 control value
        ...
        oscCountTime = map(theOscMessage.get(4).floatValue(),0,100,0,100);
        oscFloatSignal = int(map(theOscMessage.get(5).floatValue(),1,2,0,1));
        oscAccelMax = map(theOscMessage.get(6).floatValue(),2.345,200,0,200);
        oscAccelStdDev = map(theOscMessage.get(7).floatValue(),1.5,100,0,100);
        oscposXStdDev =  map(theOscMessage.get(8).floatValue(),6.27,200,0,200);
        oscposYStdDev = map(theOscMessage.get(9).floatValue(),6.16,200,0,200);
        oscConductEvents = int(theOscMessage.get(10).floatValue());
        oscConductTime = map(theOscMessage.get(11).floatValue(),0,60000,15,45000);
        oscGraphicsTrigger=map(theOscMessage.get(12).floatValue(),0.0241,25,1,5);
        oscColourTrigger=map(theOscMessage.get(13).floatValue(),1.2166,50,1,25);
        oscParticlemaxspeed = map(theOscMessage.get(14).floatValue(),0.0673,10,0.8,3);
        oscParticlemaxforce = map(theOscMessage.get(15).floatValue(),-0.00178,10,1,10);
