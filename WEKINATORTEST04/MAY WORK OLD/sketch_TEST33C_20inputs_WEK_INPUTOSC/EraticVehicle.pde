...
//
//
//   CREATE SWARM OF ERATIC PARTILCES
//
...
class EraticVehicle {
  ...
  float mousemaxforce;    // Maximum steering force
  float mousemaxspeed;    // Maximum speed
  ...
  color interA;
    ...
    EraticVehicle(float x, float y, int whatkind, float mmaxspeed, float mmaxforce,float myRadius) {
   timeToLerp=0;
  ...
  void applyBehaviors(ArrayList<EraticVehicle> vehicles,ArrayList<DarkVehicle> darkvehicles, int Mx,int My) {
   ...
   //seekForce = seek(EraticMove(anglecounter));
       }// lastpos diff from posn
       
       seekForce = seek(EraticMove(radians(anglecounter)));
     
    //seekForce = mouseSeek(seekForce);
      
     // println("....................................................");
       
      // println("...............ERATIC MOVE.............");
    // println("ERATIC MOVE",EraticMove(radians(anglecounter)));
  ...
  PVector mouseSeek(PVector target) {
    PVector desired = PVector.sub(target,position);  // A vector pointing from the position to the target
    
    // Normalize desired and scale to maximum speed
    desired.normalize();
    desired.mult(mousemaxspeed);
    // Steering = Desired minus velocity
    PVector steer = PVector.sub(desired,velocity);
    steer.limit(mousemaxforce);  // Limit to maximum steering force
...
PVector seekRandom(PVector target) {
  ...
  PVector separate (ArrayList<EraticVehicle> vehicles) {
    float desiredseparation = radius*2;
    PVector sum = new PVector();
    int count = 0;
    // For every boid in the system, check if it's too close
    for (EraticVehicle other : vehicles) {
  ...
  void display(int lerpperiod) {
   ...
   void changecol(ArrayList<EraticVehicle> vehicles,int mX,int mY ) {
...
PVector RandomMove(){
 ...
 PVector EraticMove(float anglecounter){
  ...
  //random move please
        ...
        float y = 400+ 300*sin(3*anglecounter+ 0.02);
        float x = 800+ 800*sin(2*anglecounter);
      ...
      //  y+= xs;
       // x+= ys;
        ...
        eraticPos.x=x;
        eraticPos.y=y;
       // if(eraticPos.x < 0) { eraticPos.x = 0; xs *= -.5;}
       // if(eraticPos.x > width-20) { eraticPos.x = width-20; xs *= -.5;}
        //if(eraticPos.y < 0) { eraticPos.y = 0; ys *= -.5;}
        //if(eraticPos.y > height-20) { eraticPos.y = height-20; ys *= -.5;}
